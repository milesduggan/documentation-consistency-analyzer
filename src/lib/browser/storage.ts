...
export async function updateIssueAssignee(
  ...
  const issue = await get<StoredIssue>(STORES.ISSUES, issueId);
  if (issue) {
    issue.assignee = assignee || undefined;
    await put(STORES.ISSUES, issue);
  }
}
 ...
 * Maps fingerprint -> {id, status, assignee} for looking up stored data
 */
export async function getIssueStatusMap(
  projectName: string,
  inconsistencies: Inconsistency[],
  projectPath?: string
): Promise<Map<string, { id: string; status: StoredIssue['status']; assignee?: string }>> {
  const statusMap = new Map<string, { id: string; status: StoredIssue['status']; assignee?: string }>();
  ...
  for (const inconsistency of inconsistencies) {
    ...
    if (storedIssue) {
      statusMap.set(fingerprint, { id: storedIssue.id, status: storedIssue.status, assignee: storedIssue.assignee });
    }
  }
