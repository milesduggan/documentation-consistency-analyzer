...
import { saveAnalysis, getLatestAnalysis, formatRelativeTime, getAllProjects, getIssueStatusMap, updateIssueStatus, updateIssueAssignee } from '@/lib/browser/storage';
...
export default function Home() {
  ...
  const [issueStatusMap, setIssueStatusMap] = useState<Map<string, { id: string; status: StoredIssue['status']; assignee?: string }>>(new Map());
  ...
  const handleStatusChange = async (issueId: string, fingerprint: string, status: StoredIssue['status']) => {
    try {
      ...
      // Update local state (preserve assignee)
      setIssueStatusMap((prev) => {
        const updated = new Map(prev);
        const existing = updated.get(fingerprint);
        updated.set(fingerprint, { id: issueId, status, assignee: existing?.assignee });
        return updated;
      });
    } catch (err) {
  ...
  const handleBulkStatusChange = async (updates: Array<{ issueId: string; fingerprint: string; status: StoredIssue['status'] }>) => {
    try {
      ...
      // Update local state (preserve assignees)
      setIssueStatusMap((prev) => {
        const updated = new Map(prev);
        for (const u of updates) {
          const existing = updated.get(u.fingerprint);
          updated.set(u.fingerprint, { id: u.issueId, status: u.status, assignee: existing?.assignee });
        }
        return updated;
      });
    } catch (err) {
  ...
  const handleAssigneeChange = async (issueId: string, fingerprint: string, assignee: string | undefined) => {
    try {
      ...
      setIssueStatusMap((prev) => {
        ...
        const existing = updated.get(fingerprint);
        if (existing) {
          updated.set(fingerprint, { ...existing, assignee });
        }
        return updated;
      });
    } catch (err) {
